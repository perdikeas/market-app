...
def regime_filter(df, period=50):
...
# 1. SMA Crossover
# ─────────────────────────────────────────────
def sma_crossover(df, fast=10, slow=30, use_regime_filter=True):
    close = df["close"]
...
# ─────────────────────────────────────────────
def momentum_strategy(df, lookback=20, holding_period=10, use_regime_filter=True):
    close = df["close"]
    past_return = close.pct_change(lookback)
    signals = pd.Series(0, index=df.index)

    for i in range(lookback, len(df), holding_period):
        signals.iloc[i] = 1 if past_return.iloc[i] > 0 else -1
...
def volatility_breakout(df, atr_period=14, atr_multiplier=1.5,
    ...
    sma_exit = close.rolling(exit_period).mean()   # fixed: was `period`

    daily_move = high - close.shift(1)

    signals = pd.Series(0, index=df.index)
    signals[daily_move > atr * atr_multiplier] =  1
    signals[close < sma_exit]                  = -1
...
def trend_mean_reversion(df, trend_period=100, rsi_period=14,
    ...
    close = df["close"]
    trend_sma = close.rolling(trend_period).mean()   # fixed: was df.rolling()
    rsi = _rsi_series(close, rsi_period)             # fixed: was compute_rsi(df)

    in_uptrend = close > trend_sma

    signals = pd.Series(0, index=df.index)
    # fixed: rsi_oversold is an int, can't call .shift() on it
...
def opening_range_breakout(df, range_days=5, atr_period=14,
    ...
    range_low  = low.rolling(range_days).min().shift(1)   # fixed: was .max()
...
def triple_screen(df_daily: pd.DataFrame, weekly_macd_fast=12, weekly_macd_slow=26,
    ...
    # Screen 3: close above yesterday's high
    breakout_entry = close > high.shift(1)

    signals = pd.Series(0, index=df_daily.index)   # fixed: was df.index
...
STRATEGIES = {
    ...
    "bollinger": {
        ...
        "description": "Buy on breakout above upper band. Sell on breakdown below lower band.",
        "params": {
            "period":  {"type": "int",   "default": 20,  "min": 10,  "max": 50,  "label": "Period"},
            "std_dev": {"type": "float", "default": 2.0, "min": 1.0, "max": 3.0, "label": "Std Dev"},
        }
    },
    "macd": {
        "fn": macd_strategy,
    ...
    "momentum": {
        ...
        "description": "Buy when past return is positive, hold for fixed period.",
        "params": {
            "lookback":       {"type": "int", "default": 20, "min": 5,  "max": 252, "label": "Lookback Days"},
            "holding_period": {"type": "int", "default": 10, "min": 1,  "max": 60,  "label": "Holding Period"},
        }
    },
    "multi_signal": {
        "fn": multi_signal,
    ...
    "kalman_trend": {
        ...
        "params": {
            "observation_covariance": {"type": "float", "default": 0.1,  "min": 0.01, "max": 1.0, "label": "Observation Noise"},
            "transition_covariance":  {"type": "float", "default": 0.01, "min": 0.001,"max": 0.5, "label": "Transition Noise"},
        }
    },
